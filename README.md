...
- Docker Engine with Docker Compose v2, or Docker Desktop with WSL integration.
- Git for repository updates.
- Node.js 20.19 or newer only when running development tools outside Docker.

### Fresh installation

```bash
git clone git@github.com:CheyneWeb3/Memopoly.git
cd Memopoly
cp .env.example .env
```

Before exposing the game publicly, replace all example secrets in `.env`:

```dotenv
NODE_ENV=production
COOKIE_SECURE=true
MONGO_PASSWORD=REPLACE_WITH_A_LONG_RANDOM_PASSWORD
ADMIN_KEY=REPLACE_WITH_A_LONG_RANDOM_ADMIN_KEY
SPECTATOR_IP_PEPPER=REPLACE_WITH_A_LONG_RANDOM_SECRET
```
...
```bash
docker compose ps
docker compose logs --tail=200 app game-worker caddy
```

> Do not run `docker compose down -v` unless you intentionally want to delete the persistent MongoDB, Redis and Caddy volumes.

## Cloudflare Tunnel deployment

The public beta hostname is:

```text
https://memopoly.hausserver.xyz
```

For the Docker tunnel, configure the Cloudflare Published Application route as:

```text
Hostname: memopoly.hausserver.xyz
Service:  http://caddy:80
```

Keep the Cloudflare tunnel token outside the repository. Never commit it to GitHub or place it in `README.md`, `.env.example` or Compose source.
...
- MongoDB and Redis: Docker network services
- Deadline processing: dedicated worker process

## Verification

Run the project checks before committing a release:
...
```text
apps/web                 React, Vite and browser game interface
apps/server              API, sessions, Socket.IO and deadline worker
packages/protocol        Shared schemas, events and network types
packages/game-engine     Server-authoritative board and game rules
ops/caddy                Reverse-proxy configuration
Dockerfile               Production application image
Dockerfile.web           Optional standalone web build
Dockerfile.server        Optional standalone server build
docker-compose.yml       Complete persistent game stack
```

## Security notes

- Game rules and money movement are authoritative on the server.
- The browser does not decide dice results, ownership or balances.
- Raw spectator IP addresses are not stored.
- Memopoly does not request or store cryptocurrency private keys.
- Never commit `.env`, tunnel tokens, deploy-key private files or live admin keys.
- Immediately rotate any credential accidentally pasted into a public issue, log or chat.

## Image setup

Replace this placeholder near the top of `README.md`:

```html
<img src="PASTE_GAME_IMAGE_URL_HERE" alt="Memopoly multiplayer browser game board" width="1100">
```

Example after uploading an image to GitHub or another public image host:

```html
<a href="https://memopoly.hausserver.xyz">
  <img src="https://your-public-image-url.example/memopoly-board.png" alt="Memopoly multiplayer browser game board" width="1100">
</a>
```

The surrounding `<div align="center">` keeps the image and title centred on GitHub. Clicking the image opens the live beta.
